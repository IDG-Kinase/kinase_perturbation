...
if (! file.exists(here('transcript_to_hgnc.csv'))) {
	...
	transcript_to_hgnc = readr::read_csv(here('transcript_to_hgnc.csv'))
}
```
...
run_DESeq2 <- function(drug_perturb_files) {
	...
	drug_perturb_exp = tximeta(drug_perturb_files)
	dds <- DESeqDataSet(drug_perturb_exp, design = ~treatment)
	keep <- rowSums(counts(dds)) > 1
	dds <- dds[keep,]
	
	dds_analysis <- DESeq(dds)
	dds_results = results(dds_analysis, alpha=0.05, lfcThreshold = 1)
	
	dds_results_filtered = as.data.frame(dds_results)
	dds_results_filtered$ensembl_transcript_id = rownames(dds_results)
	...
	return(data.frame(
		...
		dk_diff_str = glue_collapse(sort(dds_results_filtered_dark_kinase$hgnc_symbol), sep=" | ")
	))
}
```
