...
kinase_ratio_plot = data.frame(umap_1 = kinase_correlations_ratio$layout[,1],
														umap_2 = kinase_correlations_ratio$layout[,2],
														symbol = drug_perturb_ratio_wide_kinases$hgnc_symbol) %>%
	left_join(all_kinases %>% select(symbol, class))

library(ggrepel)
library(BerginskiRMisc)
ggplot(kinase_ratio_plot, aes(x=umap_1,y=umap_2,color = class)) +
...
```{r}
kinase_hier_clusters = as.data.frame(cutree(kinase_cluster_ratio,k = 4))
names(kinase_hier_clusters) <- "cluster_num"
kinase_hier_clusters = kinase_hier_clusters %>%
	rownames_to_column(var = "hgnc_symbol") %>%
	arrange(cluster_num, hgnc_symbol) %>%
