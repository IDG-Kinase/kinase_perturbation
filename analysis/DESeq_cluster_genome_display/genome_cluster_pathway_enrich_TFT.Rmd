---
title: "Genome Clustering - Pathway Enrichment Analysis - TFT:GTRD"
...
This preliminary report details the results of looking for pathway enrichment in each of the groups of genes produced by hierarchical clustering of the drug perturbations. I've cut the the data into `r number_of_clusters`, but this value can be changed if we decide on a different value. Otherwise, this analysis focuses on enriched pathways in the MSigDB database. Specifically, this analysis uses the C3 TFT:GTRD segment from: https://www.gsea-msigdb.org/gsea/msigdb.
...
genesets <- msigdb_gsets("Homo sapiens", "C3", "TFT:GTRD")
...
pathway_counts = all_pathway_hits %>%
	mutate(trimmed_label = str_replace(label,"HALLMARK_","")) %>%
	count(trimmed_label) %>% 
	arrange(desc(n))
pathway_counts$label = fct_relevel(pathway_counts$trimmed_label,pathway_counts$trimmed_label)

pathway_over_one = pathway_counts %>% filter(n > (pathway_counts$n[20]-1))
num_pathway_single_hit = pathway_counts %>% filter(n <= (pathway_counts$n[20]-1)) %>% pull(trimmed_label) %>% length()
```

Overall, `r dim(pathway_counts)[1]` pathways are enriched in at least one of the following clusters. Of these pathways `r num_pathway_single_hit` are hit less than `r (pathway_counts$n[20]-1)` times. The following pathways are seen in `r (pathway_counts$n[20]-1)` or more clusters:
