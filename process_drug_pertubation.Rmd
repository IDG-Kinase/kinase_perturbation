...
drug_perturb_ratio = drug_perturb %>%
	...
	ungroup()
```
Bit of code to check whether the DESeq2 result line up with my ad-hoc ratios

```{r}
temp = drug_perturb_ratio %>% 
	mutate(full_name = case_when(
		treatment == "Dina" ~ "Dinaciclib",
		treatment == "Ent" ~ "Entinostat",
		treatment == "JIB-04 low" ~ "JIB-04 (low dose)",
		treatment == "MK2206" ~ "MK2206",
		treatment == "Tram" ~ "Trametinib",
		treatment == "JIB-04 high" ~ "JIB-04 (high dose)",
		treatment == "Navi" ~ "Navitoclax",
		treatment == "Paclit" ~ "Paclitaxel",
		treatment == "Rux" ~ "Ruxolibinib",
		treatment == "SGC" ~ "SGCCP30",
		treatment == "Alis" ~ "Alisertib",
		treatment == "Bort" ~ "Bortezomib",
		treatment == "CoCl2" ~ "CoCl2",
		treatment == "FCCP" ~ "FCCP",
		treatment == "Palbo" ~ "Palbociclib"
	)) %>% select(-treatment) %>% rename(treatment = full_name) %>%
	left_join(DESeq2_estimates %>% select(log2FoldChange, batch, treatment, hgnc_symbol))
```
